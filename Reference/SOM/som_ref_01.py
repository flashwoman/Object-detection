...
## http://paraschopra.com/old-website-archive/sourcecode/SOM/index.php
...
class SOM:
    ...
    # Let radius=False if you want to auto-calculate the radius
                ...
                for k in range(self.total):
                    ...
                    if dist < radius_decaying:
                        ...
                        for l in range(self.FV_size):
                            # Learning
                            temp_FV[l] = self.nodes[k].FV[l] + influence * learning_rate_decaying * (
                                        input_FV[l] - self.nodes[k].FV[l])

                        for l in range(self.PV_size):
                            # Learning
                            temp_PV[l] = self.nodes[k].PV[l] + influence * learning_rate_decaying * (
...
if __name__ == "__main__":
    ...
    print(
    "Prediction 0 0,", round(a.predict([0, 0])[0])
    )
    print(
    "Prediction 1 0,", round(a.predict([1, 0])[0])
    )
    print(
    "Prediction 0 1,", round(a.predict([0, 1])[0])
    )
    print(
    "Prediction 1 1,", round(a.predict([1, 1])[0])
    )
